...
To develop bitcore-node-gobyte:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-gobyte.git
...
We now will setup symlinks in `bitcore-node-gobyte` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-gobyte
ln -s ~/bitcoind-rpc-gobyte
```
...
cd devnode
mkdir node_modules
touch bitcore-node-gobyte.json
touch package.json
```

Edit `bitcore-node-gobyte.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-gobyte/bin/bitcore-node-gobyte start
```